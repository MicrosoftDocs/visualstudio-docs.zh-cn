...
`Version`元素的作用域限定为特定模块及版本的可视化条目。 `Version`元素可帮助避免发生名称冲突，可减少无意匹配，并允许不同类型的版本的不同可视化效果。  

如果由不同模块使用常见头文件定义了一种类型，当类型在特定模块版本时，显示版本控制可视化效果。  

在以下示例中，仅适用于`Windows.UI.Xaml.dll`从 1.0 到 1.5 版中，找到的`DirectUI::Border`类型可视化效果。 
