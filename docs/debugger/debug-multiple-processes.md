...
###  <a name="BKMK_Automatically_start_an_process_in_the_debugger"></a> 自动在调试器中启动的进程  
 有时，你需要调试由另一个进程启动的程序启动代码。这样的示例包括服务和自定义安装操作。 在这些情况下，可以让调试器在应用程序启动时启动并自动附加。  
...
3. 选择要在调试器中启动的应用程序的文件夹。  
    ...
    如果应用的名称未作为子文件夹列出，选择 **Image File Execution Options** ，然后选择上下文菜单中 **新建** ，**密钥** 。 选择新的密钥，选择快捷菜单中的 **重命名**，然后输入应用的名称。  
  
4. 在应用文件夹的上下文菜单中，选择**新建**，**字符串值**。  
  
5. 更改新值的名称 **新值**到`debugger` 。  
...
###  <a name="BKMK_Switch_between_processes"></a> 进程之间切换  
 调试时可以附加到多个进程，但在任何给定时间，调试器中只有一个进程处于活动状态。 在调试位置工具栏或在进程**进程**窗口，可以设置活动或 *当前* 进程。 若要在两个进程间切换，这两个进程必须处于中断模式。  
  
 **若要设置当前进程**  
  
- 在调试位置工具栏上，选择 **进程** 查看**进程**列表框。 选择要指定为当前进程的进程。  
...
- 打开**进程**窗口 (快捷方式**Ctrl + Alt + Z**)，查找想要设置为当前进程的进程，然后双击它。  
  ...
  切换到一个项目设置当前进程用于调试目的。 您查看的所有调试器窗口将显示当前进程的状态，并且所有单步执行命令将仅影响当前进程。  
...
- [停止、 终止和分离命令](#BKMK_Stop__terminate__and_detach_commands)  
  ...
  默认情况下，当您在调试器中打开多个进程时，选择 **调试** 、 **停止调试** ，调试器将终止或分离所有进程，具体取决于调试器之前如何打开进程：  
  
- 如果之前在调试器中启动的当前进程，则该进程将终止。  
  
- 如果您之前附加调试器到当前进程，则调试器会与进程分离，并使进程保持运行。  
