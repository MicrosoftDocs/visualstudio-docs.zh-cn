...
1. 在 `shape.Draw()` 方法上暂停时，将鼠标悬停在 `shapes` 对象上，你会看到其默认属性值， 另一个对象类型为 `Rectangle`。

1. 展开 `shapes` 对象以查看其所有属性，例如 `Height` 属性，其值是否为0。
...
1. 在主代码编辑器窗口中，右键单击 `shapes` 对象，然后选择**添加监视**。
    ...
    **监视**窗口将在代码编辑器的底部打开。 可使用**监视**窗口指定要关注的变量（或表达式）。
    ...
    现在，你给 `shapes` 对象设置了一个监视，当你在调试器中浏览时，可看到其值发生变化。 与其他变量窗口不同，**监视** 窗口始终显示你正在监视的变量（当超出作用域时，它们会变灰）。

## <a name="examine-the-call-stack"></a>检查调用堆栈

1. 在 `foreach` 循环中暂停时，单击 **调用堆栈** 窗口，默认情况下，该窗口在右下方窗格中打开。

2. 单击 **F11** 几次，直至看到调试器在代码编辑器中`Triangle` 类的 `Base.Draw` 方法处暂停。 查看 **调用堆栈** 窗口。

    ![检查调用堆栈](../debugger/media/get-started-call-stack.png "ExamineCallStack")

    **调用堆栈** 窗口显示方法和函数被调用的顺序。 最上面一行显示当前函数（此应用中的 `Triangle.Draw` 方法）。 第二行显示 `Main` 方法中调用 `Circle.Draw` 的语句，依此类推。
   ...
   > **调用堆栈** 窗口类似于某些 IDE（如 Eclipse）中的调试透视图。
    ...
    可双击某行代码来查看源代码，这也会更改调试器正在检查的当前作用域。 此操作不会使调试器前进。
...
1. 在 `Circle.Draw` 方法调用中暂停调试器后， 使用鼠标抓住左侧的黄色箭头（执行指针），将黄色箭头向上移动一行到 `Console.WriteLine`方法调用语句上。

1. 再按一次 **F11**。

    调试器重新运行 `Console.WriteLine` 方法（你会看到控制台窗口输出消息）。
...
在本教程中，你已了解了如何启动调试器、单步跟踪代码以及检查变量。 你可能希望更深入地了解调试器功能以及查看指向更多信息的链接。
